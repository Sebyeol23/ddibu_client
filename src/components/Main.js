  ...
  async function fetchMoreData() {
    try {
      const res = await axios.get(`${apiUrl}/api/home/product`, {
        params: {
          limit: 18,
          lastId: items.length > 0 ? items[items.length - 1].productId : null,
        },
      });
      if (!res.data.length) setHasMore(false);
      else setItems([...items, ...res.data]);
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        if (res.data.length < 18) setHasMore(false);
        setItems(res.data);
      } catch (error) {
        console.log(error);
      }
        ...
        {items.length > 0 &&
          items.map((item, index) => (
            <div
              ...
              key={item.productId}
              onClick={() => navigate(`/product-info?pid=${item.productId}`)}
            >
              <div
                className={styles.image}
                style={{
                  backgroundImage: `url(data:image/${item.extension};base64,${item.image})`,
                }}
              ></div>
              <div className={styles.data}>
                <div className={styles.name}>{item.title}</div>
